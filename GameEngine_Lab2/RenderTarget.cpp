...
void RenderTarget::Render(float _deltaTime)
{
	...
	int xRes = ws.X + (NATIVE_XRES * perc); // now add 0.4444444 * NATIVE_XRES to the current resolution
	                                       // i.e., 0.4444444 * 1920 = 853 + 800 = 1653
	int yRes = NATIVE_YRES;               // same as ws.Y + (NAYIVE_YRES * perc) = 600 * 0.4444444 = 480 + 600 = 1080
